"""
Implements Q-Learning to discover optimal, coupled control parameters 
(PID gains) based on the robot's interaction with the environment.
Includes epsilon decay, physical parameter bounds, and embodied constraints.
"""
...
class QLearningTuner:
    ...
    def __init__(
        ...
        # Epsilon decay parameters for Exploration vs Exploitation
        ...
        # Define the joint action space (-1: Decrease, 0: Keep Same, 1: Increase)
    ...
    def _discretize_state(self, metrics: Dict[str, float]) -> str:
        """Dynamically discretizes state based on injected thresholds."""
        if not metrics:
            return "INIT"
    ...
    def choose_action(self, current_state: str) -> int:
        """Epsilon-greedy policy selecting a single joint-action index[cite: 706]."""
    ...
    def apply_action_to_params(self, action_idx: int):
        """Decodes the joint-action index and applies adjustments within strict bounds."""
        adjustments = self.action_space[action_idx]
        
        for i, param in enumerate(self.param_names):
            if adjustments[i] == -1:    # Decrease
                self.current_params[param] -= self.step_sizes[param]
            elif adjustments[i] == 1:   # Increase
                self.current_params[param] += self.step_sizes[param]
                
            # Strictly enforce lower and upper bounds to prevent PID explosion
            min_val, max_val = self.bounds[param]
            self.current_params[param] = max(min_val, min(max_val, self.current_params[param]))

    def decay_epsilon(self):
        """Gradually reduces exploration rate to favor exploitation over time."""
        self.epsilon = max(self.min_epsilon, self.epsilon * self.epsilon_decay)

    def update_q_table(self, reward: float, next_state: str):
        """Updates the Q-table using the standard Q-Learning equation[cite: 710]."""
...
class Learning:
    """Integrated online/offline learning system with embodied physical constraints."""
    ...
    def __init__(
        ...
        self.rewards = rewards or {
            ...
            "torque_penalty_weight": 50.0,  # High penalty for exceeding embodied torque limits
            "energy_penalty_weight": 0.1    # Small continuous penalty for inefficiency
        }
    ...
    def update_from_trial(self, trial_data: Dict[str, Any]):
        ...
        # Penalize exceeding physical robot constraints
        ...
        # 3. Discretize next state and update Q-Table
        next_state = self.tuner._discretize_state(perf)
        self.tuner.update_q_table(reward, next_state)
        
        # 4. Choose next action, apply limits, and step epsilon
    ...
    def save_state(self):
        ...
        state = {
            ...
            "epsilon": self.tuner.epsilon  # Remember decay state
        }
        try:
