...
def rgb_to_point_cloud(rgb, depth, mask, width=320, height=240, fov=60):
    """
    ...
    z = depth_meters
    x = (u - cx) * z / f
    y = -(v - cy) * z / f  # Negative because image y-axis points down
...
def detect_target_object(points, colors, target_color=[1.0, 0.0, 0.0], color_tolerance=0.5):
    ...
    """
    # Calculate color distances to find how close we are to red
    target_color_arr = np.array(target_color)
    color_diff = np.linalg.norm(colors - target_color_arr, axis=1)
    red_ish_points = np.sum(color_diff < color_tolerance)
    
    # Debug: Print color statistics occasionally
    if np.random.rand() < 0.01:  # 1% chance to print (avoid spam)
        print(f"[TargetDetect] Total points: {len(points)}, Red-ish points: {red_ish_points}, Tolerance: {color_tolerance}")
    
    # Detect red points
    target_points, target_center = detect_objects_by_color(
        points, colors, target_color, color_tolerance
    )
    
    if target_points is None or len(target_points) < 5:  # Need at least 5 points (lowered for small target)
        return None, None, None
...
def process_sensor_data(sensor_data):
    ...
    points, colors, masks = rgb_to_point_cloud(rgb, depth, mask)
