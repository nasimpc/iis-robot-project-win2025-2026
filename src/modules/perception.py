...
# ----------------------------------------------------------------------
# Global variables for static camera-to-base transform (filled by init)
_camera_to_base_rot = None   # 3x3 rotation matrix (camera frame -> base frame)
_camera_to_base_trans = None  # 3x1 translation

def init_camera_transform(robot_id, camera_link_name="camera_link"):
    """
    Compute the fixed transform from camera frame to robot base frame.
    Call this once after loading the robot, with the robot in its zero pose.
    """
    ...
    # Get link state in base frame (robot at zero pose)
    link_state = p.getLinkState(robot_id, camera_idx, computeForwardKinematics=True)
    cam_pos = link_state[0]   # position in base frame
...
def rgb_to_point_cloud(rgb, depth, mask, robot_id=None, width=320, height=240, fov=60):
    """
    Convert RGB-D image to 3D point cloud in world coordinates (if robot_id given)
    or in camera coordinates (if robot_id is None).
    ...
    Args:
        ...
        robot_id: PyBullet robot ID for coordinate transformation.
                  If None, points are returned in camera frame.
        width: Image width
        height: Image height
    ...
    # Transform from camera frame to world frame if robot_id is provided
...
def process_sensor_data(sensor_data, robot_id=None):
    ...
    # Convert to point cloud in world coordinates (if robot_id provided) or camera frame
...
def process_sensor_data_relative(sensor_data):
    """
    Processes sensor data and returns detected landmarks as relative positions
    (dx, dy) in the robot base frame (x forward, y left).

    This function is intended for state estimation (particle filter) and uses
    the static camera-to-base transform precomputed by init_camera_transform().

    Args:
        sensor_data: Dict containing 'rgb', 'depth', 'mask'

    Returns:
        dict with keys:
            'target': (dx, dy) or None
            'obstacles': list of (dx, dy) for each obstacle in order [Blue, Pink, Orange, Yellow, Black]
                         (None if not detected)
            'table': (dx, dy) or None (if table detection is implemented)
    """
    global _camera_to_base_rot, _camera_to_base_trans
    ...
    # Get point cloud in camera frame (robot_id=None)
    points_cam, colors, _ = rgb_to_point_cloud(rgb, depth, mask, robot_id=None)

    # Detect target (returns centroid in camera frame)
    ...
    if target_center_cam is not None:
        ...
        target_rel = (target_base[0], target_base[1])  # (dx, dy)
    else:
        target_rel = None

    # Detect obstacles (returns list of dicts with 'center' in camera frame)
    obstacles_cam = detect_obstacles(points_cam, colors)
    obstacles_rel = []
    # The detect_obstacles returns list in the order of iteration over obstacle_colors,
    # which is the same order as the default dict (Python 3.7+ preserves insertion order).
    ...
    # Table detection could be added similarly, but requires adapting detect_table to camera frame.
